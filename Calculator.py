...
window = Tk()
window.geometry("352x424") # width and height
window.resizable(0,0) # no resizing
window.title("Calculator")


# updates continously input field
def btn_click(item):
...
# calaculates the expression    
def btn_equal():
    global expression
    try:
        result = str(eval(expression)) # eval evaluates expression
...
input_text = StringVar() # to get the instance of input field


# creating frame for input field
...
btns_frame = Frame(window,width =  312,height = 275.5,bg = 'grey' )
btns_frame.pack()
